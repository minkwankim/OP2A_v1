...
void precessingGridFLUENT(const  std::string& mesh_file_name_Fluent)
{
	...
	std::vector<std::vector<int> > face_node(nfm, std::vector<int>(GRID_FACE_MAXNNODES, -1));
			...
			if (zone > 0)
			{
				...
				case 0:
					for (int i = index_s; i <= index_e; i++)
					{
						...
						std::vector<int> temp_nodeList (GRID_FACE_MAXNNODES, -1);
						int cl = -1;
						int cr = -1;
	...
	std::vector<std::vector<int> > cell_NodeList(ncm, std::vector<int>(GRID_CELL_MAXNNODES, -1));
	std::vector<std::vector<int> > cell_FaceList(ncm, std::vector<int>(GRID_CELL_MAXNFACES, -1));
		...
		if (cr != -1)
		{
			...
			case HEXA8:
				...
				else
				{
					...
					if (faceComp(temp_face2, face_node[f], QUAD4) == 1)
					{
						cell_FaceList[c][2]	= face_ID[f];
						filled_faces[c] += 1;
					}

					if (faceComp(temp_face4, face_node[f], QUAD4) == 1)
					{
						cell_FaceList[c][4]	= face_ID[f];
						filled_faces[c] += 1;
					}

					if (faceComp(temp_face5, face_node[f], QUAD4) == 1)
					{
			...
			case PRISM6:
				...
				else if (face_type[f] == QUAD4)
				{
					...
					else if (filled_nodes[c] == 6)
					{
						...
						if (faceComp(temp_face3, face_node[f], QUAD4) == 1)
						{
							cell_FaceList[c][3]	= face_ID[f];
							filled_faces[c] += 1;
						}
			...
			case PYRAMID5:
				...
				else if (face_type[f] == TRI3)
				{
					...
					else if (filled_nodes[c] == 5)
					{
						...
						if (faceComp(temp_face1, face_node[f], TRI3) == 1)
						{
							cell_FaceList[c][1]	= face_ID[f];
							filled_faces[c] += 1;
						}

						if (faceComp(temp_face2, face_node[f], TRI3) == 1)
						{
							cell_FaceList[c][2]	= face_ID[f];
							filled_faces[c] += 1;
						}

						if (faceComp(temp_face3, face_node[f], TRI3) == 1)
						{
							cell_FaceList[c][3]	= face_ID[f];
							filled_faces[c] += 1;
						}

						if (faceComp(temp_face4, face_node[f], TRI3) == 1)
						{
							cell_FaceList[c][4]	= face_ID[f];
							filled_faces[c] += 1;
						}
