	...
	for (int f = 0; f <= nfm-1; f++)
	{
		if (face_include[f] == true)
		{
			for (int f1 = 0; f1 <= nfm-1; f1++)
			{
				...
				{
					switch (faceComp(face_node[f], face_node[f1], face_type[f]))
					{
					case 1:
	...
	while (! mesh_file_SU2.eof())
	{
		...
		if (readIntTemp != -1)
		{
			for (int bc = 0; bc <= nbc-1; bc++)
			{
				...
				{
					int temp_BC_EleType;
					std::vector<int> temp_BC_EleNodeList(GRID_FACE_MAXNNODES);
					...
					for (int f1 = 0; f1 <= nfm-1; f1++)
					{
						...
						{
							if (faceComp(face_node[f1], temp_BC_EleNodeList, temp_dummy) != 0)
							{
